...
A. QMD line numbers where at least 3 data sources are imported: 45, 74, 105

## Data Wrangling / Cleaning / etc.

**DO AT LEAST FOUR OF THE FOLLOWING:**

A. QMD line number(s) for general data wrangling: 46-51

B. QMD line number(s) for a join operation: 88

C. QMD line number(s) for `pivot_wider` or `pivot_longer` operation (or equivalent):

D. QMD line number(s) for use of regular expressions: 85

E. QMD line number(s) for use of reduction and/or transformation functions: 86
...
A. QMD line number(s) for **all data visualizations**: 62, 92, 129

B. QMD line number(s) for multiple different geoms: 

C. QMD line number(s) for use of multiple asthetics: 62
...
A. Include QMD line numbers for **all tables**: 98, 135
